...
- **按住说话** — 按住热键录音，**立即显示预览窗口**，实时显示中间结果，松开 1.5 秒后自动输出
- **自动适配** — 兼容所有 Windows 麦克风设备，自动重采样到 16kHz
- **波纹可视化** — 屏幕顶部小窗口实时显示语音波纹，右键切换样式或退出
- **文字预览** — Win32 原生窗口渲染中文，超过 20 字自动滚动
- **自学习纠错** — 按 Esc 纠正识别错误，系统自动学习：corrections + bigram 频率 + 热词 + 上下文窗口 + 同音泛化
- **纠错优先级** — corrections 始终覆盖 bigram 统计，用户纠正永久生效
...
| **按住热键** | 按住 F1（可配置）开始录音，**立即显示预览窗口**，实时显示中间识别结果 |
| **松开热键** | 停止录音，识别结果经纠错+标点后显示 **1.5 秒**，然后自动输出到光标位置 |
| **按 Esc** | 在 1.5 秒预览期间按 Esc 弹出纠错对话框，修改后系统自动学习 |
...
│  ～～∿∿～～∿～～∿∿～～∿～～～  │
├─────────────────────────────────────────┤
│  我今天想讨论一下关于人工智能的方案      │  ← 实时文字预览
└─────────────────────────────────────────┘
                    ↓ 松开 → 纠错+标点 → 1.5秒预览（按Esc可纠正）
┌─────────────────────────────────────────┐
│  我今天想讨论一下关于人工智能的方案。    │  ← 最终结果（自动输出到光标）
└─────────────────────────────────────────┘
                    ↓
┌─────────────────────────────────────────┐
│  ●─────────────────────────────────     │  ← 回到就绪
...
纠错流水线的执行顺序保证 corrections 是最终权威：

```
ASR 原始输出 → Bigram 统计纠错（猜测性修正）→ corrections 精确覆盖（用户/自动学习的规则，最终权威）
```

即使 Bigram 把某个字改错了，corrections 仍会在最后把它纠正回来。用户的每次 Esc 纠正都会永久生效。
